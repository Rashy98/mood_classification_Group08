...
from arch.fuse_attlstm import fuse_att
import random
...
if __name__ == '__main__':
    ...
    parser.add_argument('--best_model_path', default='fuseatt_marousal.pth')
    ...
    sptf_x = np.load('/home/k/kzheng3/Final/data/spotify/seg_features_normalized.npy')
    print(args.emo_dim)
    ...
    df.to_csv('fuse_att_spotify_spect_'+args.emo_dim+'_test.csv',index = False)
    tacc = np.sum(np.array(gt)==np.array(preds))/len(meta_test)
    gt, preds = predict(args,model,valid_dataloader)
    df = pd.DataFrame({'gt':gt,'pred':preds})
    df.to_csv('fuse_att_spotify_spect_'+args.emo_dim+'_valid.csv',index = False)
