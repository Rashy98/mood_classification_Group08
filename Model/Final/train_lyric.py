...
from arch.model_new import MultiNet
import random
...
def predict(args,model, testset_loader):
    ...
    with torch.no_grad():
        for vstep, (data,labels) in enumerate(tqdm(testset_loader)):
            input_tensor =  data.to(args.device)#data.unsqueeze(1).to(args.device)
            label = labels.type(torch.LongTensor).to(args.device)

            vlogits = model(input_tensor)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch_num', default=200, help='number of train epochs')
    parser.add_argument('--device', default='cuda', help='cuda device')
    parser.add_argument('--learning_rate', default=6e-5, help='learning rate')
    parser.add_argument('--best_model_path', default='lyric_arousal.pth')
    ...
    print(args.emo_dim)
    all_x = np.load('./data/standardized_lyrics.npy')
    train_set = lyricDataset(all_x,meta_train[target_cols],0)
    valid_set = lyricDataset(all_x,meta_valid[target_cols],len(meta_train))
    test_set = lyricDataset(all_x,meta_test[target_cols],len(meta_train)+len(meta_valid))

    train_dataloader = DataLoader(train_set, batch_size=16, shuffle=True,drop_last=True,num_workers=0 )
    valid_dataloader = DataLoader(valid_set,batch_size = 16,shuffle = True,num_workers=0 )
    test_dataloader = DataLoader(test_set, batch_size=1, shuffle=True,num_workers=0 )


    model = MultiNet()
    # if args.emo_dim=='arousal':
    #     model.load_state_dict(torch.load('./pretrained/arousal.pt'),strict = False)
    # else:
    #     model.load_state_dict(torch.load('./pretrained/valence.pt'),strict = False)

    if args.is_test:
        best_stat = torch.load(os.path.join(args.best_model_path),strict = False)
    else:
   ...
   #df.to_csv('mlp.csv',index = False)
    ...
    tracc = np.sum(np.array(gt)==np.array(preds))/len(train_dataloader.dataset)

    gt, preds = predict(args,model,test_dataloader)
    tacc = np.sum(np.array(gt)==np.array(preds))/len(test_dataloader.dataset)
    gt, preds = predict(args,model,valid_dataloader)
    acc = np.sum(np.array(gt)==np.array(preds))/len(valid_dataloader.dataset)
