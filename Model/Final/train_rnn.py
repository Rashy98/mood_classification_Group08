...
from arch.rnn import RNN_encoder
import random
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch_num', default=150, help='number of train epochs')
    parser.add_argument('--device', default='cuda', help='cuda device')
    parser.add_argument('--learning_rate', default=8e-5, help='learning rate')
    parser.add_argument('--best_model_path', default='rnn_arousal.pth')
    ...
    all_x = np.load('/home/k/kzheng3/Final/data/spotify/seg_features_normalized.npy')
    print(args.emo_dim)


    train_set = cnnDataset(all_x,meta_train[target_cols],0)
    valid_set = cnnDataset(all_x,meta_valid[target_cols],len(meta_train))
    test_set = cnnDataset(all_x,meta_test[target_cols],len(meta_train)+len(meta_valid))
    ...
    #     model.load_state_dict(torch.load('./pretrained/rnn_arousal.pth'))
    # else:
    #     model.load_state_dict(torch.load('./pretrained/cnn_valence.pth'))

    if args.is_test:
        best_stat = torch.load(os.path.join(args.best_model_path))
    else:
   ...
   # df.to_csv('rnn_spect_'+args.emo_dim+'_test.csv',index = False)
    tacc = np.sum(np.array(gt)==np.array(preds))/len(meta_test)
    gt, preds = predict(args,model,valid_dataloader)
   # df = pd.DataFrame({'gt':gt,'pred':preds})
  #  df.to_csv('rnn_spect_'+args.emo_dim+'valid.csv',index = False)
