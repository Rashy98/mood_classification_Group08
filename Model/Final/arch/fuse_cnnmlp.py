...
class fuse_cs_direct(nn.Module):#input_shape= (224,224,3)
    ...
    def forward(self, spect,sptf):
        ...
        # print(torch.amax(spect_fet),torch.amax(sptf_fet))
        # print(torch.amin(spect_fet),torch.amin(sptf_fet))
...
if __name__ == '__main__':
    ...
    stat = torch.load('/home/k/kzheng3/Final/arch/cnn_valence.pth')
    # final_stat = stat[stat.keys()[:-2]]
    # print(final_stat.keys())
    sptf = torch.ones(1,33)
   # model.spect_model.load_state_dict(stat,strict=False)
    spects = torch.ones((1,3,224,224))#torch.rand(size=(1, 3, 224, 224))
    model(spects,sptf)
    # logits = model(spects,sptf) # # 1,64,14,14
