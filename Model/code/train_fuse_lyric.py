...
from arch.fuse_ls import fuse_ls
import random
...
if __name__ == '__main__':
    ...
    sptf_x = np.load('./data/standardized_lyrics.npy')
    print(args.emo_dim)
    ...
    # if args.emo_dim=='arousal':
    #     model.lyric_model.load_state_dict(torch.load('./pretrained/arousal.pt'),strict=False)
    # else:
    #     model.lyric_model.load_state_dict(torch.load('./pretrained/valence.pt'),strict=False)
    model.train()
    ...
    df.to_csv('fuse_spotify_spect_valence_test.csv',index = False)
    tacc = np.sum(np.array(gt)==np.array(preds))/len(meta_test)
    gt, preds = predict(args,model,valid_dataloader)
    df = pd.DataFrame({'gt':gt,'pred':preds})
    df.to_csv('fuse_spotify_spect_valence_valid.csv',index = False)
