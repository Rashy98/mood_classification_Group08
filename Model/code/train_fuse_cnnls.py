...
from arch.fuse_cnnlstm import fuse_att
import random
...
if __name__ == '__main__':
    ...
    parser.add_argument('--best_model_path', default='fusels_marousal.pth')
    ...
    if args.emo_dim=='arousal':
        ...
        model.sptf_fc.load_state_dict(torch.load('./pretrained/mlp_arousal.pth',strict=False))
    else:
        model.spect_model.load_state_dict(torch.load('./pretrained/cnn_valence.pth'),strict=False)
        model.sptf_fc.load_state_dict(torch.load('./pretrained/mlp_valence.pth',strict=False))
    model.train()
